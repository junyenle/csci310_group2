...
public class CollageBuilderServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String shapeText = request.getParameter("shapeText").toUpperCase(); // get shape text
		...
		CollageOptions options = new CollageOptions(collageBorderWidth, collageBorderColor, photoBorderWidth, photoBorderColor, minRotation, maxRotation, collageWidth, collageHeight, filter);
		...
		if(s.isValid()) // if image sourcing worked, build collage
		{
			...
			CollageBuilder collageBuilder = new CollageBuilder(options, browserWidth, browserHeight); // Instantiate CollageBuilder object
			
			BufferedImage collage = collageBuilder.buildCollage(s, shapeText);			// CollageBuilder.buildCollage(imageSource) builds a collage out of the 30
